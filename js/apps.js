...
export function renderApps(cms,q=''){
 ...
 document.getElementById('appsGrid').innerHTML=`<div class="apps-heading"><strong>${esc(cat)}</strong><small>${count} accesos listos para usar</small></div>` + (rows.map(x=>`<a class="app-card fun-card" href="${esc(first(x,['URL','Url','Link']))}" target="_blank" rel="noopener"><span>${esc(first(x,['Icono'])||'🔗')}</span><div><strong>${esc(first(x,['Nombre','App','Herramienta']))}</strong><small>${esc(labelCat(first(x,['Categoria','Categoría']))||'Acceso')}</small></div><b class="arrow">›</b></a>`).join('') || '<p class="muted">Sin resultados.</p>');
}
